...
score-max-score ``float``
  *С версии 1.5.1*
  ...
  Максимальное значение **score**, при котором метод **is_alive** возвращает **true**. **score** рассчитывается как сумма оценок загрузки сети, сессий и средней загрузки вычислительных. Каждая из оценок рассчитывается как *(N/N_max) \* 10,0*, где *N* - текущее занчение параметра, N_max - максимальное. Также **is_alive** возвращает **false** при превышении хотя бы одной оценкой значения 8,0.

score-max-net-load ``integer``
  *С версии 1.5.1*
  
  Максимальная загрузка исходящего сетевого потока в MiB.

score-max-sessions ``integer``
  *С версии 1.5.1*
  
  Максимальное количество входящих TCP-сессий.

score-max-cpu-la1 ``float``
  *С версии 1.5.1*
  
  Максимальное значение средней загрузки вычислительных ресурсов за 1 минуту.
